...
# Some quick methods.
apply_immediate = lambda hist: (len(hist['objects']) == 1 and
                                hist['mergeable'] and
                                'exist' not in hist['template'])
apply_group = lambda hist: (len(hist['objects']) >= 2 and
                            hist['mergeable'] and
...
def caption(scene, templates):
  ...
  sample_captions = sample_from_hypotheses(caption_hypotheses, scene, templates)
